...
<br>
High altitude reduces air density, which means there is less air resistance on the baseball. As a result, the ball can travel farther when hit, and pitches tend to have less movement. Both effects can lead to more scoring and more home runs.

Coors Field is located in Denver at about 5,200 feet above sea level, making it the highest-elevation stadium in Major League Baseball. Because of this altitude, we would expect Coors Field to have higher average total runs, and possibly more home runs, than other stadiums.
...
<br>
There are 2430 games in the dataset. We aggregate individual games into stadium averages before we start the visualization process because it allows us to see the average number of runs scored by each stadium in a given season. This is useful because it allows us to compare the performance of different stadiums across a season.
:::
...
<br>
A horizontal bar chart is better than a scatter plot for this data because it is easier to compare the performance of different stadiums across a season. A scatter plot would not be as easy to compare because it would be difficult to see the difference between the performance of different stadiums.
- When would you choose a vertical bar chart over horizontal?
<br>
A vertical bar chart would be better than a horizontal bar chart when you want to compare the performance of different stadiums across a season. 
...
<br>
The color highlighting in Approach 4 accomplishes the goal of highlighting Coors Field as the most run-friendly ballpark in 2010. The position (x/y) is used to highlight the ordinality of the data. (If you are higher up on the y-axis, you are more run-friendly than if you are lower on the y-axis.)
...
<br>
The visualization is suitable for a business presentation because it is professional and easy to understand. The coloring of COL is helpful because it instantly makes the point of the graph jump out at the reader: COL is the most run friendly park in 2010. Additionally, The way we use y axis positioning is extremely intuitive, making the graph do most of the work, not necessarily the reader. The annotation is helpful because it gives extra information to the reader about the stadium we care about the most: Colorado..
...
for index, (season, runs_col) in enumerate([(2010, "runs_2010"), (2021, "runs_2021")]):
    ...
    colorArray = [
        "darkorchid" if stadium == "COL" else "lightgrey"
        for stadium in groupDF.home
    ]
